...
function generateImage({ data, premium, ppt, pt, cagr, maturity }) {
  // A4 Portrait @300 DPI
  const width = 2480;
  const height = 3508;
  ...
  ctx.fillText("Funds can be withdrawn anytime after 5 years", width / 2, 292);
  ...
  ctx.fillText(`Premium Paying Term : ${ppt} Years`, 120, 470);

  ctx.fillText(`Policy Term : ${pt} Years`, 120, 530);

  ctx.fillText(`Projected CAGR : ${cagr}%*`, 120, 590);
  ...
  // Header
  ctx.fillStyle = "#fff";
  ctx.font = "bold 39px Roboto"; // was 34 (+15%)
  ctx.textAlign = "left";
  ...
  // Body
  ctx.font = "bold 34px Roboto"; // was 30 (+13-15%)
  ...
  data.forEach((row, i) => {
    ...
    ctx.fillRect(100, y, width - 200, rowHeight);

    ctx.fillStyle = "#000";

    ctx.fillText(row.Year.toString(), 170, y + 40);

    ctx.fillText(row.Investment.toLocaleString("en-IN"), 520, y + 40);

    ctx.fillText(row.FundValue.toLocaleString("en-IN"), 1430, y + 40);
